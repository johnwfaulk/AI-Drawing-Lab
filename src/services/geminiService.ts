...
import { GameCategory, AIModel, DrawingDifficulty } from '../types'; 
import { 
    TEXT_MODEL_ID, 
    CONCEPT_GENERATION_PROMPT_TEMPLATE, 
    DRAWING_GENERATION_PROMPT_TEMPLATE, 
    IMAGE_MODELS, 
    DIFFICULTY_MODIFIERS,
    SYSTEM_INSTRUCTION_FOR_IMAGE_MODEL // Import new constant
} from '../constants';
...
export const generateDrawingFromAPI = async (
  ...
  let userSpecificPromptContent = DRAWING_GENERATION_PROMPT_TEMPLATE(concept);
  
  if (selectedModel.promptModifier) {
    userSpecificPromptContent = selectedModel.promptModifier(userSpecificPromptContent);
  }

  userSpecificPromptContent += ` ${DIFFICULTY_MODIFIERS[difficulty]}`; 

  // Prepend the system instruction to the user-specific prompt content.
  const fullPrompt = `${SYSTEM_INSTRUCTION_FOR_IMAGE_MODEL} ${userSpecificPromptContent}`;

  try {
    const response = await client.models.generateImages({
        model: imageModelId,
        prompt: fullPrompt, // Use the combined prompt
        // system_instruction is not a valid parameter here. 
        // System-level instructions are included in the 'fullPrompt'.
    ...
    console.error("Full prompt sent to API:", fullPrompt); 
    // Removed logging for SYSTEM_INSTRUCTION_FOR_IMAGE_MODEL separately as it's now part of fullPrompt
    if (error instanceof Error) {
